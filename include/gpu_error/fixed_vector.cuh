#ifndef ERROR_FIXED_VECTOR
#define ERROR_FIXED_VECTOR
...
__global__ void copy_vector_to_dev_array(T *dev_array, vector *dev_vector,
                                         uint64_t size) {
  uint64_t tid = gallatin::utils::get_tid();
...
//   -- additional workers are automatically pulled into the environment to
//   assist resizes.
//   == TODO: allow for dynamic parallelism if #workers crosses lower threshold?

// T - type inserted and retreived from the vector
// Allocator - allocator type - this is Gallatin normally, but let's expose it
// to other systems. resizable - allow the vector to be resized
...
struct fixed_vector {
  using my_type = fixed_vector<T, min_items, max_items>;

  static const uint n_directory =
      (gallatin::utils::numberOfBits(max_items - 1) + 1 -
       gallatin::utils::numberOfBits(min_items - 1));
  ...
  T *directory[(gallatin::utils::numberOfBits(max_items - 1) -
                gallatin::utils::numberOfBits(min_items - 1) + 1)];
  ...
  __device__ bool add_new_backing(uint64_t directory_index) {
    if (atomicOr((unsigned long long int *)&locks,
                 (unsigned long long int)SET_BIT_MASK(directory_index)) &
        SET_BIT_MASK(directory_index))
      return false;
    ...
    T *new_backing = (T *)hmalloc(sizeof(T) * new_size);

    // printf("Ending malloc\n");
    // atomicExch breaks here?
    atomicCAS((unsigned long long int *)&directory[directory_index],
              (unsigned long long int)0ULL,
              (unsigned long long int)new_backing);
  ...
  // not threadsafe so only one thread can call this safely.
  ...
  __device__ uint64_t get_local_position(uint64_t clipped_hash,
                                         uint64_t index) {
  ...
  __device__ uint64_t insert(T item) {
    ...
    if (global_read_directory == nullptr) {
      ...
      while (global_read_directory == nullptr) {
        global_read_directory = (T *)gallatin::utils::ld_acq(
            (uint64_t *)&directory[directory_index]);
      }
    }
  ...
  __device__ uint64_t bulk_insert(cg::coalesced_group &active_threads, T item) {
    ...
    my_group_sum =
        cg::exclusive_scan(active_threads, my_group_sum, cg::plus<uint64_t>());
  ...
  // deference operator - double check that memory has been cleared before
  // giving back.
  ...
  // static __host__ std::vector<T> export_to_host_device(my_type *
  // device_version){
  ...
  // 		T * host_array =
  // gallatin::utils::copy_to_host<T>(host_version->directory[directory_index],
  // n_items);
  ...
  // //host variant
  // static __host__ std::vector<T> export_to_host_host(my_type *
  // device_version){
  ...
  static __host__ std::vector<T> export_to_host(my_type *device_version) {
    ...
    copy_vector_to_dev_array<T, my_type>
        <<<(size - 1) / 256 + 1, 256>>>(device_array, device_version, size);

    cudaDeviceSynchronize();

    GPUErrorCheck(cudaMemcpy(host_vector.data(), device_array, sizeof(T) * size,
