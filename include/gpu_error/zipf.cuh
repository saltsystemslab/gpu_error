...
uint64_t zipf(double alpha, uint64_t n, bool reset=false)
{
  ...
  if (first == true)
  {
    ...
    {
    progress_bar c_bar("Calculate Zipfian C probabilities", n, .001);
    for (i=1; i<=n; i++){
    ...
    {
    progress_bar prob_bar("Assigning Zipfian probabilities", n, .001);
    for (i=1; i<=n; i++) {
      sum_probs[i] = sum_probs[i-1] + c / pow((double) i, alpha);
      prob_bar.increment(1);
    }
    first = false;
    }
...
__host__ uint64_t * generate_zipfian_values(uint64_t items_to_generate, uint64_t max_range, double alpha){
   ...
   if (std::filesystem::exists(output_fname)){
      ...
      progress_bar zipf_bar("Zipfian gen", items_to_generate, .001);
